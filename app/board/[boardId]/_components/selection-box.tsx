...
interface SelctionBoxProps {
  onResizeHandlePointerDown: (corner: Side, initalBounds: XYWH) => void;
}

const HANDLE_WIDTH = 8;

export const SelectionBox = memo(({ onResizeHandlePointerDown }: SelctionBoxProps) => {
  const soleLayerId = useSelf((me) => (me.presence.selection.length === 1 ? me.presence.selection[0] : null));
  const isShowingHanldes = useStorage((root) => soleLayerId && root.layers.get(soleLayerId)?.type !== LayerType.Path);
  ...
  return (
    <>
      <rect
        className="fill-transparent stroke-amber-500 stroke-2 pointer-events-none"
        style={{
